...
pdf(file.path(workdir,"Figure3a.pdf"),  onefile=FALSE, width=16, height=8)

forestplot(labeltext=tabletext, 
           graphwidth = unit(90, 'mm'),
           graph.pos=6,
           is.summary=c(TRUE,TRUE, rep(FALSE, 6),TRUE,rep(FALSE, 6),TRUE,rep(FALSE, 8)),
          
           mean=c(NA,data$RR), 
           lower=c(NA,data$down), upper=c(NA,data$up),
          
          title ="Incident hepatocellular carcinoma in patients with type 2 diabetes, with and without MASLD/MASH/liver fibrosis and cirrhosis\n(Comparison between propensity-score matched cohorts)", 
           ...
           hrzl_lines=list("2" = gpar(lwd = 1.2, lty=1, col='black'),
                          ...
                          "17" = gpar(lwd = 0.9, lty='longdash', col='black')
